...
struct ToggleVoiceSessionIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        ...
        // Post notification for in-process delivery (openAppWhenRun = true
        // causes perform() to execute in the app's process).
        NotificationCenter.default.post(
            name: Notification.Name("remclaw.didOpenURL"),
            object: URL(string: "remclaw://voice/\(action)")!
        )

        // Reload the control widget to reflect the new state
