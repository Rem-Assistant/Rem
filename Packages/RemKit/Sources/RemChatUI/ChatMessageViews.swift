...
struct ChatMessageBubble: View {
    ...
    let style: RemGatewayChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? RemChatTheme.userText : RemChatTheme.assistantText
    ...
    private var inlineAttachments: [RemChatMessageContent] {
    ...
    private var toolCalls: [RemChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [RemChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? RemChatTheme.userBubble
        }
        if self.style == .onboarding {
            return RemChatTheme.onboardingAssistantBubble
        }
        return RemChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return RemChatTheme.onboardingAssistantBorder
        }
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? RemChatTheme.userText : RemChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(RemChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [RemChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: RemChatTheme.assistantText)
            }
        }
    }
}
