...
enum RemPlatformImageFactory {
    static func image(_ image: RemGatewayPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
