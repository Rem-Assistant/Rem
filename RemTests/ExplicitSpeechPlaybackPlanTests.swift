...
struct ExplicitSpeechPlaybackPlanTests {
    ...
    @Test func executorStopCancelsLaterChunksAndFailureBecomesRetryableOnlyInSameSession() async {
        ...
        let standaloneRetry = ExplicitSpeechRetryToken(
            text: chunks.joined(separator: " "),
            context: context,
            generation: 7,
        ...
        #expect(standaloneRetry.usesSystemVoiceOnly)
        #expect(ExplicitSpeechRetryPolicy.canRetry(
            standaloneRetry,
            expectedContext: context,
            currentGeneration: 7,
        ...
        #expect(DailyBriefReadControlState.resolve(
            isReading: false,
            canRetry: true
        ) == .retry)
        #expect(DailyBriefReadControlState.retry.title(
