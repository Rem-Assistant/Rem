...
// Exercises `RemConversationApiService` against a mock transport: it asserts the exact HTTP method,
// path (including query), and request body for each call, that success bodies decode into the
// domain result types, and that non-2xx responses map to `RemConversationApiError.requestFailed`
// carrying the backend's structured `reason` / `failure_code`.

@MainActor
private final class MockConversationHttpClient: RemConversationHttpClient {
...
@MainActor
@Suite("RemConversationApiService")
struct RemConversationApiServiceTests {
    ...
    private func makeService(_ mock: MockConversationHttpClient) -> RemConversationApiService {
        RemConversationApiService(http: mock)
    }
    ...
    func sendChatRequestAndDecode() async throws {
        let mock = MockConversationHttpClient()
        let idempotencyKey = "1a2b3c4d-0000-4000-8000-0000000000ff"
        mock.handler = { _, _, _ in
            (Self.chatResponseJSON.data(using: .utf8)!, MockConversationHttpClient.response(201))
        }
    ...
    func sendChatQuotaFailure() async throws {
        ...
        mock.handler = { _, _, _ in
            ...
            return (body.data(using: .utf8)!, MockConversationHttpClient.response(429))
        }
        let service = makeService(mock)
    ...
    func approveRequestAndDecode() async throws {
        let mock = MockConversationHttpClient()
        mock.handler = { _, _, _ in
            (Self.approveResponseJSON.data(using: .utf8)!, MockConversationHttpClient.response(200))
        }
        let service = makeService(mock)
    ...
    func approveTaskChangedConflict() async throws {
        ...
        mock.handler = { _, _, _ in
            ...
            return (body.data(using: .utf8)!, MockConversationHttpClient.response(409))
        }
        let service = makeService(mock)
    ...
    func dismissSuccess() async throws {
        let mock = MockConversationHttpClient()
        mock.handler = { _, _, _ in (Data(), MockConversationHttpClient.response(204)) }
        let service = makeService(mock)
    ...
    func dismissNotFound() async throws {
        let mock = MockConversationHttpClient()
        mock.handler = { _, _, _ in
            (#"{"error":"Task proposal not found"}"#.data(using: .utf8)!, MockConversationHttpClient.response(404))
        }
        let service = makeService(mock)
    ...
    func getConversationRequestAndDecode() async throws {
        let mock = MockConversationHttpClient()
        mock.handler = { _, _, _ in
            (Self.historyResponseJSON.data(using: .utf8)!, MockConversationHttpClient.response(200))
        }
        let service = makeService(mock)
    ...
    func createConversationEmptyBody() async throws {
        let mock = MockConversationHttpClient()
        mock.handler = { _, _, _ in
            (Self.conversationJSON.data(using: .utf8)!, MockConversationHttpClient.response(201))
        }
        let service = makeService(mock)
    ...
    func listConversationsDecode() async throws {
        ...
        mock.handler = { _, _, _ in
            ...
            return (body.data(using: .utf8)!, MockConversationHttpClient.response(200))
        }
        let service = makeService(mock)
    ...
    func malformedSuccessBody() async throws {
        let mock = MockConversationHttpClient()
        mock.handler = { _, _, _ in ("not json".data(using: .utf8)!, MockConversationHttpClient.response(200)) }
        let service = makeService(mock)
