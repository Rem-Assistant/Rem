...
struct RemChatTransportAgentEventTests {
    @MainActor
    @Test func taskHistoryLoadsFromRemTranscriptWhenGatewayIsUnavailable() async throws {
        let taskID = "11111111-1111-4111-8111-111111111111"
        let sessionKey = "rem-task-\(taskID)"
        let transport = IOSGatewayChatTransport(
            ...
            priorTranscriptProvider: { requestedSession in
                #expect(requestedSession == sessionKey)
                return [AnyCodable([
                    "role": "assistant",
                    "content": [["type": "text", "text": "Durable Rem reply"]],
                ])]
            }
        )
        ...
        #expect(history.sessionKey == sessionKey)
        #expect(history.messages?.count == 1)
    }

    @MainActor
    @Test func ordinaryHistoryStillPropagatesGatewayFailure() async {
        let transport = IOSGatewayChatTransport(
            gateway: GatewayNodeSession(),
            chatLifecycleRequester: { _, _, _ in throw URLError(.cannotConnectToHost) },
            priorTranscriptProvider: { _ in [] }
        )

        await #expect(throws: URLError.self) {
            _ = try await transport.requestHistory(sessionKey: "ordinary-chat")
        }
    }

    @MainActor
    @Test func taskContinuationUsesRemBackendAndEmitsDurableFinalWithoutGatewayDispatch() async throws {
        let taskID = "11111111-1111-4111-8111-111111111111"
        let sessionKey = "rem-task-\(taskID)"
        let dispatchID = "33333333-3333-4333-8333-333333333333"
        var gatewayDispatchCount = 0
        let providerCalls = TaskContinuationRecorder()
        let transport = IOSGatewayChatTransport(
            gateway: GatewayNodeSession(),
            onChatSendDispatched: { gatewayDispatchCount += 1 },
            taskContinuationProvider: { session, message, idempotencyKey in
                await providerCalls.record(session, message, idempotencyKey)
                return TaskChatContinuationResult(
                    ...
                    message: TaskChatMessage(
                        ...
                        taskId: taskID,
                        role: "assistant",
                        content: "Start with the oldest thread.",
                        runId: idempotencyKey
                    )
                )
            }
        ...
        let finalEvent = Task<RemChatEventPayload?, Never> {
            for await event in stream {
                if case let .chat(payload) = event, payload.runId == dispatchID {
                    return payload
                }
            }
            return nil
        ...
        let event = await finalEvent.value

        #expect(response.runId == dispatchID)
        #expect(response.status == "completed")
        #expect(event?.state == "final")
        #expect(event?.sessionKey == sessionKey)
        #expect(gatewayDispatchCount == 0)
        #expect(await providerCalls.count == 1)
        #expect(await providerCalls.firstMessage == "What first?")
    }

    @MainActor
    @Test func abortingTaskContinuationCancelsBackendRequestWithoutGatewayAbort() async {
        let blocker = TaskContinuationRequestBlocker()
        ...
        let transport = IOSGatewayChatTransport(
            ...
            taskContinuationProvider: { _, _, _ in
        ...
        try? await transport.abortRun(sessionKey: sessionKey, runId: dispatchID)
    ...
    @MainActor
    @Test func abortBeforeTaskContinuationRegistrationPreventsBackendDispatch() async {
        let registrationBlocker = ChatSendPreflightBlocker()
        let requester = OrderedChatLifecycleRequester()
        let providerCalls = TaskContinuationRecorder()
        let sessionKey = "rem-task-11111111-1111-4111-8111-111111111111"
        let dispatchID = "44444444-4444-4444-8444-444444444444"
        let transport = IOSGatewayChatTransport(
            gateway: GatewayNodeSession(),
            chatLifecycleRequester: { method, paramsJSON, _ in
                try await requester.request(method: method, paramsJSON: paramsJSON)
            ...
            taskContinuationProvider: { sessionKey, message, idempotencyKey in
                await providerCalls.record(sessionKey, message, idempotencyKey)
                return nil
            },
            onTaskContinuationRegistered: { await registrationBlocker.wait() }
        )
        ...
        #expect(await registrationBlocker.waitUntilStarted())
        try? await transport.abortRun(sessionKey: sessionKey, runId: dispatchID)
        ...
        #expect(await providerCalls.count == 0)
        #expect(await requester.methods.isEmpty)
    }

    @MainActor
    @Test func taskConversationHealthDoesNotCallGateway() async throws {
        let requester = OrderedChatLifecycleRequester()
        let sessionKey = "rem-task-11111111-1111-4111-8111-111111111111"
        let transport = IOSGatewayChatTransport(
            gateway: GatewayNodeSession(),
        ...
        #expect(try await transport.requestHealth(timeoutMs: 500))
        #expect(await requester.methods.isEmpty)
    }

    @Test func gatewayHealthEventsCannotDisableTaskRuntimeConversation() {
        #expect(IOSGatewayChatTransport.effectiveHealth(
            gatewayOK: false,
            activeSessionKey: "rem-task-11111111-1111-4111-8111-111111111111"
        ))
        #expect(!IOSGatewayChatTransport.effectiveHealth(
            gatewayOK: false,
...
private actor TaskContinuationRequestBlocker {
    private var didStart = false
    private var startWaiters: [UUID: CheckedContinuation<Bool, Never>] = [:]
...
private actor TaskContinuationRecorder {
    private var calls: [(String, String, String)] = []

    var count: Int { calls.count }
    var firstMessage: String? { calls.first?.1 }

    func record(_ sessionKey: String, _ message: String, _ idempotencyKey: String) {
        calls.append((sessionKey, message, idempotencyKey))
    }
}
