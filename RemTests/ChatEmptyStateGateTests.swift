...
struct ChatEmptyStateGateTests {
    ...
    // MARK: - FIX 3 (#1371): a fresh conversation never flashes the prior chat's transcript

    @Test func freshConversationLoadingOverStaleMessagesShowsStartersNotTheStaleTranscript() {
        // The reused view model still holds the PRIOR conversation's rows for the frame between
        // `sessionKey` flipping and the new session's bootstrap replacing `messages`. A fresh
        // conversation has no history to fetch, so while that first load is in flight the stale
        // rows must not paint — drop straight to the starters instead of flashing "Pastor Chika".
        #expect(resolve(
            messagesEmpty: false,
            isLoading: true,
            isFreshConversation: true,
    ...
    @Test func freshConversationAfterInitialLoadShowsItsOwnTranscriptEvenWhileRefreshing() {
        // Once the fresh conversation's first bootstrap has completed, its rows are its own. A
        // later in-conversation refresh (isLoading true again) must keep the transcript, not blank
    ...
    @Test func freshConversationPendingPromptStillSuppressesStaleTranscriptWhileLoading() {
        // Arriving into a fresh chat with a prefilled composer over stale rows: the prefill states
        // intent (no starters) but the stale transcript must still not leak — transcript body above
        // the prefilled composer, never the prior conversation's messages.
        #expect(resolve(
            messagesEmpty: false,
            isLoading: true,
            isFreshConversation: true,
