...
struct MessageCleanerTests {
    ...
    @Test func stripsStandaloneRemSkillJSONManifest() {
        let input = #"""
        {
          "openclaw": {
            "emoji": "🐙",
    ...
    @Test func keepsFencedRemSkillJSONExample() {
        let input = #"""
    ...
    // MARK: - Run-now task-agent scaffolding (#1371)

    /// Mirrors the wire shape backend/src/services/task-agent.service.ts dispatches for a cloud run:
    /// `SYSTEM_PROMPT + "\n\n" + buildUserPrompt(...)`. The exact system/context prose is elided;
    /// what the cleaner keys on is the stable first clause plus the trailing INSTRUCTION line.
    private func taskAgentRunPrompt(instruction: String) -> String {
        """
        You are Rem's task agent, working on ONE task for the person who owns this device. You are \
        given the task, its prior comments, and an optional instruction. Reply with a short, \
        actionable comment (1-3 sentences) about what should happen next on this task.

        TASK: Attend RRHS Chat
        STATUS: todo
        PRIORITY: high
    ...
    @Test func stripsRunNowScaffoldingDownToTheHumanInstruction() {
        let raw = taskAgentRunPrompt(instruction: "Draft an agenda for the meeting")
        #expect(MessageCleaner.cleanUserMessageText(raw) == "Draft an agenda for the meeting")
    }

    @Test func bareRunNowScaffoldingCollapsesToNoUserBubble() {
        let raw = taskAgentRunPrompt(instruction: "(none — propose the next sensible action)")
        #expect(MessageCleaner.cleanUserMessageText(raw) == "")
    }

    @Test func ordinaryUserMessageMentioningInstructionIsUntouched() {
        // The scaffolding stripper anchors on the task-agent system prompt, so a normal chat that
        // merely uses the word "instruction" must survive verbatim.
        let input = "Can you follow the instruction I gave earlier?"
        #expect(MessageCleaner.stripTaskAgentRunScaffolding(input) == input)
        #expect(MessageCleaner.cleanUserMessageText(input) == input)
    }
}
