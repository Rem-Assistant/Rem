...
struct TaskChatSessionIdentityTests {
    ...
    @Test func canonicalTaskContinuationRoutesToBackendWithStableDispatchIdentity() async throws {
        let service = RecordingTaskCommentService()
        let coordinator = TaskChatTranscriptCoordinator(service: service)
        let dispatchID = "33333333-3333-4333-8333-333333333333"

        let result = try await coordinator.continueConversation(
            sessionKey: "rem-task-\(taskId.lowercased())",
            message: "What should I do next?",
            idempotencyKey: dispatchID
        )

        #expect(result?.runId == dispatchID)
        #expect(service.continuations == [
            .init(
    ...
    @Test func ordinaryContinuationDoesNotCallTaskBackend() async throws {
        let service = RecordingTaskCommentService()
        let coordinator = TaskChatTranscriptCoordinator(service: service)
        ...
        #expect(service.continuations.isEmpty)
    }
    ...
    @Test func epochSecondsAndMillisecondsShareOneChronology() throws {
        ...
        let decoded = try JSONDecoder().decode(RemChatMessage.self, from: data)
