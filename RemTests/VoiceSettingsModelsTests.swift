...
struct VoiceSettingsModelsTests {
    ...
    /// #1372 three-line rows: name (title) / description (subtitle) / up to three
    /// personality traits (third line). `displaySubtitle` is the prose descriptor,
    /// and `displayTraits` raises the trait cap from two to three — distinct from
    /// `displayDetail`, which stays at two for the single-line overview subtitle.
    @Test func voiceExposesSubtitleAndUpToThreeTraitsForThreeLineRow() {
        let full = VoiceSettingsVoice(
            id: "adam",
            name: "Adam",
            category: nil,
            ...
            locale: nil,
            gender: nil,
            personalities: ["Dominant", "Firm", "Deep", "Warm"]
        )
        let traitsOnly = VoiceSettingsVoice(
            id: "nova",
            name: "Nova",
            category: nil,
            description: nil,
            locale: nil,
            gender: nil,
            personalities: ["Bright", "Clear"]
        )

        #expect(full.displaySubtitle == "Dominant Firm")
        #expect(full.displayTraits == "Dominant · Firm · Deep")
        // `displayDetail` must stay capped at two, unchanged by the new line.
        #expect(full.displayDetail == "Dominant Firm")
        #expect(traitsOnly.displaySubtitle == nil)
        #expect(traitsOnly.displayTraits == "Bright · Clear")
    }
