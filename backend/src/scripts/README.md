...
| `run-orchestrator-sweep.ts` | Off-by-default scheduled task sweep. Runs a trusted-automation Rem observe turn, verifies its durable proposal, and executes one policy-approved audited `tasks.update`; runtime and claim misses are reported separately. It does not call or wake an OpenClaw gateway. |
| `reconcile-rem-tool-effects.ts` | Marks expired Rem tool effects uncertain and resolves adapter-proven task updates without redispatching them. |
