...
enum DailyBriefReadControlState: Equatable {
    case start
    case stop
    case retry

    static func resolve(isReading: Bool, canRetry: Bool) -> Self {
        if isReading { return .stop }
        if canRetry { return .retry }
        return .start
    }

    func title(hasCompletedPlayback: Bool) -> String {
        switch self {
        case .stop: return "Stop reading"
        case .retry: return "Try reading again"
        case .start: return hasCompletedPlayback ? "Read again" : "Read latest brief"
        }
    }

    func accessibilityValue(hasCompletedPlayback: Bool) -> String {
        switch self {
        case .stop: return "Reading"
        case .retry: return "Reading failed; retry available"
        case .start: return hasCompletedPlayback ? "Completed" : "Not read"
        }
    }
}
...
struct DailyBriefCard: View {
    ...
    init(
        ...
        self.canRetryPlayback = canRetryPlayback
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: DesignTokens.Spacing.sm) {
            ...
            .accessibilityHint(onTap == nil ? "Conversation will be available after delivery" : "Opens your daily brief")
    ...
    private var readActionTitle: String {
        DailyBriefReadControlState.resolve(
            isReading: isReading,
            canRetry: canRetryPlayback
        ).title(hasCompletedPlayback: hasCompletedPlayback)
    }
    ...
    private var readActionAccessibilityValue: String {
        DailyBriefReadControlState.resolve(
            isReading: isReading,
            canRetry: canRetryPlayback
        ).accessibilityValue(hasCompletedPlayback: hasCompletedPlayback)
    }
