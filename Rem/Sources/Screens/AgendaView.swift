...
public struct AgendaView: View {
    ...
    var canRetryBriefPlayback: Bool = false
    ...
    public init(
        ...
        canRetryBriefPlayback: Bool = false,
        ...
        self.canRetryBriefPlayback = canRetryBriefPlayback
        self.hasCompletedBriefPlayback = hasCompletedBriefPlayback
        self.onReadBrief = onReadBrief
    }

    /// A card can exist before its conversation does. Only a server-advertised session key makes
    /// the chat tap actionable; read-aloud and card prose remain available without one.
    private func handleBriefTap() {
        guard let onOpenBriefChat, let sessionKey = viewModel.brief?.briefSessionKey else { return }
        onOpenBriefChat(sessionKey)
    }
    ...
    private func briefTapAction(for brief: DailyBrief) -> (() -> Void)? {
        guard brief.briefSessionKey != nil else { return nil }
        return { handleBriefTap() }
    }
    ...
    private var dailyBriefCard: some View {
        if isToday, let brief = viewModel.brief, brief.hasAgendaSurface {
            DailyBriefCard(
                brief: brief,
                onTap: briefTapAction(for: brief),
                isReading: isReadingBrief,
                canRetryPlayback: canRetryBriefPlayback,
