...
struct ExplicitSpeechRetryToken: Equatable {
    ...
    let generation: UInt
    let resumesListeningAfterPlayback: Bool
    let usesSystemVoiceOnly: Bool

    init(
        text: String,
        context: ExplicitSpeechRetryContext,
        generation: UInt,
        resumesListeningAfterPlayback: Bool = true,
        usesSystemVoiceOnly: Bool = false
    ) {
        self.text = text
        self.context = context
        self.generation = generation
        self.resumesListeningAfterPlayback = resumesListeningAfterPlayback
        self.usesSystemVoiceOnly = usesSystemVoiceOnly
    }
}
...
enum ExplicitSpeechRetryPolicy {
    static func canRetry(
        ...
        guard let token, let expectedContext else { return false }
        guard voiceSessionEnabled || !token.resumesListeningAfterPlayback else { return false }
...
/// Voice mode manager for Rem using the TalkMode pattern:
...
///
/// Adapted from OpenClaw's TalkModeManager with Rem-specific gateway integration.
...
final class RemTalkModeManager: NSObject {
                ...
                for await evt in stream {
                    ...
                    struct ChatEvent: Decodable {
                        ...
                        init(from decoder: Decoder) throws {
                            ...
                            self.state = try c.decodeIfPresent(RemKit.AnyCodable.self, forKey: .state)
                        }
                    }
    ...
    private func streamAssistant(
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload, as: RemAgentEventPayload.self) else { continue }
    ...
    func readAloud(
        ...
        let completed = await withTaskCancellationHandler {
            await playAssistant(
                ...
                usesSystemVoiceOnly: usesSystemVoiceOnly
            )
        } onCancel: {
            Task { @MainActor [weak self] in
        ...
        guard !Task.isCancelled, isCurrentExplicitPlayback(generation) else { return false }
        explicitPlaybackTask = nil
        isReadingAloud = false
        if !completed, let retryContext {
            failedExplicitReading = ExplicitSpeechRetryToken(
                text: trimmed,
                context: retryContext,
                generation: generation,
                resumesListeningAfterPlayback: false,
                usesSystemVoiceOnly: usesSystemVoiceOnly
            )
    ...
    func retryFailedReadingAloud(expectedContext: ExplicitSpeechRetryContext?) async -> Bool {
        ...
        clearFailedExplicitReading()
        if failed.resumesListeningAfterPlayback {
            return await startByReadingAloud(text, retryContext: context)
        }
        return await readAloud(
            text,
            retryContext: context,
            usesSystemVoiceOnly: failed.usesSystemVoiceOnly
        )
    }
