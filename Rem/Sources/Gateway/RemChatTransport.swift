...
/// gateway's execution run id. `RemChatViewModel` filters those events against its history
...
/// Local terminal events for task turns completed synchronously by the Rem backend.
/// The chat UI already understands the gateway event vocabulary, so the transition
/// adapter emits the same final marker after the durable transcript commit.
nonisolated final class TaskChatEventRelay: @unchecked Sendable {
    private let lock = NSLock()
    private var continuations: [UUID: AsyncStream<RemChatTransportEvent>.Continuation] = [:]

    func register(_ continuation: AsyncStream<RemChatTransportEvent>.Continuation) -> UUID {
        let id = UUID()
    ...
    func yield(_ event: RemChatTransportEvent) {
        let targets = lock.withLock { Array(continuations.values) }
        targets.forEach { $0.yield(event) }
    }
}

/// Owns cancellable backend task sends by the same stable identity the chat UI uses
/// for its pending run. Cancelling a task conversation never falls through to a
/// gateway `chat.abort` for an unrelated runtime.
nonisolated final class TaskChatSendCoordinator: @unchecked Sendable {
    struct Key: Hashable { let sessionKey: String; let runID: String }
    private enum Entry {
        case prepared(Date)
        case running(Task<TaskChatContinuationResult?, Error>)
        case cancelled(Date)
    }
    private let lock = NSLock()
    private var entries: [Key: Entry] = [:]
    private let cancellationLifetime: TimeInterval = 300

    func prepare(_ key: Key) {
        lock.withLock {
            pruneCancelledEntries(now: Date())
            if entries[key] == nil { entries[key] = .prepared(Date()) }
        }
    }

    /// Claims the dispatch identity before its unstructured Task is created. This is the boundary
    /// that prevents a Stop received during view-model preparation from racing a backend call.
    func claim(_ key: Key) -> Bool {
        lock.withLock {
            pruneCancelledEntries(now: Date())
            if case .cancelled? = entries[key] {
                entries.removeValue(forKey: key)
                return false
            }
            entries[key] = .prepared(Date())
            return true
        }
    }

    func register(
        _ task: Task<TaskChatContinuationResult?, Error>,
        for key: Key
    ) -> Bool {
        lock.withLock {
            pruneCancelledEntries(now: Date())
    ...
    func cancel(_ key: Key) -> Bool {
        let task: Task<TaskChatContinuationResult?, Error>? = lock.withLock {
            pruneCancelledEntries(now: Date())
            let task: Task<TaskChatContinuationResult?, Error>?
            if case let .running(runningTask)? = entries[key] { task = runningTask } else { task = nil }
            entries[key] = .cancelled(Date())
            return task
        }
        task?.cancel()
        return true
    ...
    private func pruneCancelledEntries(now: Date) {
        entries = entries.filter { _, entry in
            switch entry {
            case let .prepared(preparedAt), let .cancelled(preparedAt):
                return now.timeIntervalSince(preparedAt) < cancellationLifetime
            case .running:
                return true
            }
        }
        if entries.count > 256 {
            let inactive = entries.compactMap { key, entry -> (Key, Date)? in
                switch entry {
                case let .prepared(date), let .cancelled(date): return (key, date)
                case .running: return nil
                }
            }.sorted { $0.1 < $1.1 }
            for (key, _) in inactive.prefix(entries.count - 256) {
                entries.removeValue(forKey: key)
            }
        }
    }
...
/// Keeps the continuation body suspended until its cancellable Task has been published to the
/// coordinator. Cancellation between claim and registration therefore cannot reach HTTP dispatch.
private actor TaskChatSendStartGate {
...
struct IOSGatewayChatTransport: RemChatTransport, Sendable {
    ...
    /// Handles only canonical task sessions on the Rem-owned runtime. `nil` means
    /// this is not a task session and the gateway path remains authoritative.
    private let taskContinuationProvider: (@Sendable (
        _ sessionKey: String,
        _ message: String,
        _ idempotencyKey: String
    ) async throws -> TaskChatContinuationResult?)?
    /// Test synchronization seam at the exact registered-before-start boundary.
    private let onTaskContinuationRegistered: (@Sendable () async -> Void)?
    private let taskEventRelay = TaskChatEventRelay()
    private let taskSendCoordinator = TaskChatSendCoordinator()
    ...
    init(
        ...
        taskContinuationProvider: (@Sendable (
            _ sessionKey: String,
            _ message: String,
            _ idempotencyKey: String
        ) async throws -> TaskChatContinuationResult?)? = nil,
        onTaskContinuationRegistered: (@Sendable () async -> Void)? = nil,
        ...
        self.taskContinuationProvider = taskContinuationProvider
        self.onTaskContinuationRegistered = onTaskContinuationRegistered
    ...
    /// History payload re-encode shape. RemChatHistoryPayload has no public
    /// memberwise init across the module boundary, so we round-trip a merged payload
    /// through JSON. Fields mirror RemChatHistoryPayload exactly.
    ...
    /// sessions.patch under the "label" key, but RemChatSessionEntry
    ...
    /// canonical gateway form to the bare alias, so `RemChatViewModel`'s
    ...
    static func normalizingCanonicalSessionKey(
        ...
        guard let newData = try? JSONSerialization.data(withJSONObject: dict),
              let decoded = try? JSONDecoder().decode(RemChatEventPayload.self, from: newData)
        else { return payload }
        return decoded
    }
    ...
    /// Rewrite a live agent event to the stable session id expected by `RemChatViewModel`.
    ...
    static func normalizingAgentRunID(
        _ payload: RemAgentEventPayload,
        sessionId: String?
    ) -> RemAgentEventPayload {
        guard let sessionId,
    ...
    static func browserToolActivity(
        from payload: RemAgentEventPayload,
        sessionKey: String
    ) -> BrowserToolActivity? {
    ...
    func abortRun(sessionKey: String, runId: String) async throws {
        if TaskChatSessionIdentity.taskId(from: sessionKey) != nil {
            _ = taskSendCoordinator.cancel(.init(sessionKey: sessionKey, runID: runId))
            return
        }
    ...
    func listSessions(limit: Int?) async throws -> RemChatSessionsListResponse {
    ...
    private func listSessionsInBoundedPages(
        targetLimit: Int
    ) async throws -> RemChatSessionsListResponse {
        var sessions: [RemChatSessionEntry] = []
        var firstResponse: RemChatSessionsListResponse?
        var lastResponse: RemChatSessionsListResponse?
        ...
        while sessions.count < targetLimit {
            ...
            let decoded = try JSONDecoder().decode(RemChatSessionsListResponse.self, from: processed)
        ...
        return RemChatSessionsListResponse(
    ...
    private func listSessionsLegacy(limit: Int?) async throws -> RemChatSessionsListResponse {
        ...
        do {
            ...
            let response = try JSONDecoder().decode(RemChatSessionsListResponse.self, from: processedRes)
    ...
    func setActiveSessionKey(_ sessionKey: String) async throws {
        // Switch the event route BEFORE any session patch can suspend. `RemChatViewModel`
    ...
    func requestHistory(sessionKey: String) async throws -> RemChatHistoryPayload {
        ...
        #endif
        state.activate(sessionKey: sessionKey)
        // A canonical task conversation is owned by Rem's backend transcript. Load that
        // source first and let gateway history be best-effort migration recovery only: a
        // missing or disconnected OpenClaw gateway must not make the current task thread
        // unreadable. Ordinary chats retain the strict gateway path below.
        if TaskChatSessionIdentity.taskId(from: sessionKey) != nil,
           let priorTranscriptProvider {
            let prior = try await priorTranscriptProvider(sessionKey)
            do {
                ...
                return try mergePriorMessages(prior, into: compatible)
            } catch is CancellationError {
                throw CancellationError()
            } catch {
                // Legacy gateway recovery is additive. The durable Rem transcript remains
                // a complete, usable task chat even when no gateway exists for this account.
                return try historyPayload(
                    sessionKey: sessionKey,
                    sessionId: nil,
                    messages: prior,
                    thinkingLevel: nil
    ...
    private func requestGatewayHistory(sessionKey: String) async throws -> RemChatHistoryPayload {
        struct Params: Codable { var sessionKey: String }
        let data = try JSONEncoder().encode(Params(sessionKey: sessionKey))
        let json = String(data: data, encoding: .utf8)
        let response = try await requestChatLifecycle(
            method: "chat.history",
            paramsJSON: json,
            timeoutSeconds: 15
        )
        return try JSONDecoder().decode(RemChatHistoryPayload.self, from: response)
    }
    ...
    private func mergeCompatibilityGatewayHistory(
        into payload: RemChatHistoryPayload,
        sessionKey: String
    ) async throws -> RemChatHistoryPayload {
    ...
    private func mergePriorTranscript(
        ...
        return try mergePriorMessages(prior, into: payload)
    }

    private func mergePriorMessages(
        _ prior: [AnyCodable],
        into payload: RemChatHistoryPayload
    ) throws -> RemChatHistoryPayload {
        guard !prior.isEmpty else { return payload }

        // Durable Rem turns and pre-migration gateway history can interleave in either
        // order. Merge by structured timestamps rather than assuming one source always
        // happened first. New task sends never append to the gateway source.
        let merged = MergedHistoryPayload(
            sessionKey: payload.sessionKey,
            sessionId: payload.sessionId,
    ...
    private func historyPayload(
        ...
        let payload = MergedHistoryPayload(
            sessionKey: sessionKey,
            sessionId: sessionId,
            messages: messages,
    ...
    func sendMessage(
        ...
        attachments: [RemChatAttachmentPayload]) async throws -> RemChatSendResponse
    {
        if TaskChatSessionIdentity.taskId(from: sessionKey) != nil, !attachments.isEmpty {
            throw NSError(
                domain: "RemTaskChat",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Task conversations do not support attachments yet."]
            )
        }
        if let taskContinuationProvider {
            let taskKey = TaskChatSendCoordinator.Key(
                sessionKey: sessionKey,
                runID: idempotencyKey
            )
            guard taskSendCoordinator.claim(taskKey) else {
                throw CancellationError()
            }
            let startGate = TaskChatSendStartGate()
            let continuationTask = Task {
                await startGate.wait()
                try Task.checkCancellation()
                return try await taskContinuationProvider(sessionKey, message, idempotencyKey)
            }
            guard taskSendCoordinator.register(continuationTask, for: taskKey) else {
                continuationTask.cancel()
                await startGate.open()
                throw CancellationError()
            }
            await onTaskContinuationRegistered?()
            await startGate.open()
            defer { taskSendCoordinator.finish(taskKey) }
            guard let result = try await continuationTask.value else {
                return try await sendMessageThroughGateway(
                    sessionKey: sessionKey,
                    message: message,
                    thinking: thinking,
                    idempotencyKey: idempotencyKey,
            ...
            guard result.sessionKey == sessionKey, result.runId == idempotencyKey else {
                throw NSError(
                    domain: "RemTaskChat",
                    code: 2,
                    userInfo: [NSLocalizedDescriptionKey: "Rem returned a mismatched task conversation receipt."]
                )
            }
            SessionLastMessageTimes.touch(sessionKey)
            SessionLastMessagePreviews.setPreview(result.message.content, for: sessionKey)
            let eventData = try JSONSerialization.data(withJSONObject: [
                "runId": result.runId,
                "sessionKey": result.sessionKey,
                "state": "final",
            ])
            let event = try JSONDecoder().decode(RemChatEventPayload.self, from: eventData)
            taskEventRelay.yield(.chat(event))
            return try JSONDecoder().decode(
                RemChatSendResponse.self,
    ...
    private func sendMessageThroughGateway(
        sessionKey: String,
        message: String,
        thinking: String,
        idempotencyKey: String,
        attachments: [RemChatAttachmentPayload]
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [RemChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let response = try JSONDecoder().decode(RemChatSendResponse.self, from: res)
    ...
    func observeSendPreparation(
        ...
        if TaskChatSessionIdentity.taskId(from: sessionKey) != nil, phase == .started {
            taskSendCoordinator.prepare(.init(sessionKey: sessionKey, runID: idempotencyKey))
        }
    ...
    func listModelCatalog() async throws -> RemChatModelCatalogSnapshot {
        #if DEBUG
    ...
    static func decodeModelCatalog(from data: Data) throws -> RemChatModelCatalogSnapshot {
        ...
        return RemChatModelCatalogSnapshot(
            models: models,
    ...
    private static func catalogCompleteness(
        from wireValue: Bool?
    ) -> RemChatModelCatalogCompleteness {
        switch wireValue {
        case true: .complete
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        if let sessionKey = state.route?.sessionKey,
           TaskChatSessionIdentity.taskId(from: sessionKey) != nil {
            return true
        }
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(RemGatewayHealthOK.self, from: res))?.ok ?? true
    }

    static func effectiveHealth(gatewayOK: Bool, activeSessionKey: String?) -> Bool {
        guard let activeSessionKey,
              TaskChatSessionIdentity.taskId(from: activeSessionKey) != nil else {
            return gatewayOK
        }
        return true
    }
    ...
    func events() -> AsyncStream<RemChatTransportEvent> {
        ...
        return AsyncStream(RemChatTransportEvent.self, bufferingPolicy: .bufferingNewest(200)) { continuation in
            let taskRelayID = taskEventRelay.register(continuation)
            let task = Task {
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: RemGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: Self.effectiveHealth(
                            gatewayOK: ok,
                            activeSessionKey: state.route?.sessionKey
                        )))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let decodedChatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: RemChatEventPayload.self)
                        {
                            ...
                            // event to RemChatViewModel. The gateway
                    ...
                    case "agent":
                        ...
                        guard let decodedPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: RemAgentEventPayload.self) else { break }
            ...
            continuation.onTermination = { @Sendable _ in
                task.cancel()
                taskEventRelay.unregister(taskRelayID)
            }
        }
    }
    ...
    private static func agentEventLooksToolRelated(_ payload: RemAgentEventPayload) -> Bool {
    ...
    private func injectSessionDisplayNames(
        _ response: RemChatSessionsListResponse
        ...
        do {
            ...
            return try JSONDecoder().decode(RemChatSessionsListResponse.self, from: patchedData)
        } catch {
            #if DEBUG
