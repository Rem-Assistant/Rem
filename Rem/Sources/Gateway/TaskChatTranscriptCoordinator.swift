...
final class TaskChatTranscriptCoordinator {
    ...
    /// objects (the shape `RemChatHistoryPayload.messages` decodes), oldest-first.
    ...
    /// Route a composer send only when the session key belongs to a task. Returning
    /// `nil` is the explicit signal for the transport to keep using its legacy path
    /// for ordinary chat, voice, browser, and other gateway sessions.
    func continueConversation(
        ...
        return try await service.continueChat(
            taskId: taskId,
            message: message,
    ...
    /// decodes as an `RemChatMessage` (role + a single text content block).
    private static func historyMessage(from message: TaskChatMessage) -> AnyCodable {
        ...
        if let createdAt = message.createdAt,
           let date = TaskComment.parseISO8601(createdAt) {
            // RemChatMessage timestamps are epoch milliseconds (the optimistic
