...
/// Manages two gateway connections for Rem:
///
...
actor RemGatewayClient {
    ...
    /// An unexpected socket drop reported by RemKit's disconnect callback
    ...
    ///     a short-lived bootstrap credential and routed to RemKit's
    ...
    /// caller). RemKit's `selectConnectAuth` (`GatewayChannel.swift:498`)
    ...
    /// Forgets the device-auth pairing tokens stored by RemKit, then
