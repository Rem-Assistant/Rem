...
enum NodeInvocationRouter {
    ...
    private static func buildRegistry() {
        ...
        // System
        h[RemSystemCommand.notify.rawValue] = SystemCommandHandler.handleNotify
        // Closure wrapper needed: handleWhich is sync but Handler typealias is async
        h[RemSystemCommand.which.rawValue] = { req in SystemCommandHandler.handleWhich(req) }
        // Internal dispatch wrapper (gateway v2026.4.9+) — not a tool itself.
        h[RemSystemCommand.run.rawValue] = handleSystemRun
        nonAdvertised.insert(RemSystemCommand.run.rawValue)
        addStub(RemSystemCommand.execApprovalsGet.rawValue, "Exec approvals are not applicable on iOS")
        addStub(RemSystemCommand.execApprovalsSet.rawValue, "Exec approvals are not applicable on iOS")
        ...
        for cmd in [RemCameraCommand.list, .snap, .clip] {
            addStub(cmd.rawValue, "Camera is not yet implemented in Rem. Coming soon.")
        }
        addStub(RemScreenCommand.record.rawValue, "Screen recording is not yet implemented in Rem. Coming soon.")
        ...
        for cmd in [RemCanvasA2UICommand.push, .pushJSONL, .reset] {
        ...
        h[RemCanvasCommand.present.rawValue] = CanvasCommandHandler.handlePresent
        h[RemCanvasCommand.hide.rawValue] = CanvasCommandHandler.handleHide
        addUnadvertised(RemCanvasCommand.navigate.rawValue, CanvasCommandHandler.handleNavigate)
        addUnadvertised(RemCanvasCommand.evalJS.rawValue, CanvasCommandHandler.handleEval)
        addUnadvertised(RemCanvasCommand.snapshot.rawValue, CanvasCommandHandler.handleSnapshot)
    ...
    private static func handleSystemRun(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        guard let paramsJSON = req.paramsJSON,
            ...
            return BridgeInvokeResponse(
                id: req.id, ok: false,
                error: RemNodeError(code: .invalidRequest, message: "system.run requires command"))
        }
