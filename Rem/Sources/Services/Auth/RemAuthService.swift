...
final class RemAuthService {
    ...
    private static func prepareAuthRefreshForDebugBackendIfNeeded() -> Bool {
        ...
        print("[Rem] debug backend changed; cleared auth and gateway credentials for \(currentBackendURL)")
        return true
    }
    ...
    private static func prepareGatewayRefreshForDebugBackendIfNeeded() -> Bool {
        ...
        GatewayConfigStore(keychainService: "app.remclaw").removeAll()
        print("[Rem] debug backend changed; cleared stale gateway credentials for \(currentBackendURL)")
        return true
    }
    #endif
    ...
    func checkStoredToken() async {
        ...
        do {
            ...
            print("[Rem] restoreGatewayCredentialsIfNeeded failed during token restore: \(error.localizedDescription)")
        }
    ...
    func restoreGatewayCredentialsIfNeeded() async throws {
        guard RemCredentialStore.backendToken != nil else {
            print("[Rem] restoreGatewayCredentialsIfNeeded skip: no backend token")
            return
        }
        ...
        do {
            ...
            print("[Rem] /me/credentials response status=\(http.statusCode)")
            ...
            print("[Rem] gateway credentials restored url=\(creds.gatewayUrl)")
        } catch let error as AuthError {
            print("[Rem] restoreGatewayCredentialsIfNeeded storage failed: \(error.localizedDescription)")
            throw error
        } catch is AuthenticatedHttpError {
            // 401 handled by AuthenticatedHttpClient (sign-out triggered automatically)
            print("[Rem] restoreGatewayCredentialsIfNeeded: auth error")
        } catch {
            print("[Rem] restoreGatewayCredentialsIfNeeded failed: \(error.localizedDescription)")
        }
    }
