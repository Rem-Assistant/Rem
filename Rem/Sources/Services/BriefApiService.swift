...
public struct DailyBrief: Codable, Sendable, Hashable {
    ...
    /// True when `markdown` is a canonical backend-authored artifact. Conversation delivery is a
    /// separate capability: an authored card may be visible before a gateway transcript exists.
    ...
    /// Whether today's brief has a truthful, displayable Agenda surface. A Rem-authored artifact
    /// owns its card independently of gateway delivery; the session key separately authorizes a
    /// conversation tap. Legacy delivered payloads remain supported during rollout.
    public var hasAgendaSurface: Bool {
        let hasDurableConversation = briefSessionKey?.trimmingCharacters(in: .whitespacesAndNewlines) ==
            DailyBriefTranscriptReconciler.durableSessionKey
        guard isAuthored || hasDurableConversation else { return false }
