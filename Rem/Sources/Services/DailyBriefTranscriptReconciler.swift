...
enum DailyBriefTranscriptReconciler {
    ...
    /// `/brief` always carries deterministic Agenda prose. Explicit backend `is_authored` authority
    /// permits direct card narration without a gateway; the durable key separately authorizes
    /// transcript reconciliation for conversational playback.
    static func backendAuthorizedCanonicalMarkdown(from brief: DailyBrief) -> String? {
        let hasDurableConversation = brief.briefSessionKey?
            .trimmingCharacters(in: .whitespacesAndNewlines) == durableSessionKey
        guard brief.isAuthored || hasDurableConversation else { return nil }
