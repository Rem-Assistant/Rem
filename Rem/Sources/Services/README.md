...
> The backbone of the iOS app. All the business logic that isn't UI lives here — signing in with Apple or Google, reading and writing your calendar and reminders, syncing tasks with the backend, scheduling notifications, managing focus sessions, handling subscriptions, and tracking usage.

Business logic services for authentication, calendar, reminders, tasks, focus sessions, notifications, telemetry, in-app purchases, and usage tracking.
...
├── IAP/
│   └── IAPService.swift         # StoreKit 2 subscription management
├── Usage/
...
- StoreKit 2 transaction observation and purchase flow
- Backend entitlement sync (`POST /api/v1/iap/transaction-sync`)
- App Account Token linking for server-side validation
- Purchase completion, deferred `Transaction.updates`, restore, reconcile, and launch/current-
  entitlement scans all publish through one serialized revision coordinator. Already-queued work
  receives the coordinator in FIFO order, canceled waiters are removed without executing, and only
  a newer committed revision retires older delayed work—a failed attempt cannot suppress a
  recoverable scan. Account reset/replacement retires queued work, and every operation reports
  typed committed, stale, or failed state. Restore captures immutable account authority before
  `AppStore.sync()` but allocates its mutation revision after sync returns, so transaction updates
  emitted during sync normally commit first and the authoritative restore scan remains eligible
  afterward. If actor scheduling assigns an emitted update the next revision instead, restore and
  native purchase reconciliation accept that update only when it fully committed entitlement +
  usage for the exact same captured lifecycle and request authority.
- `IAPService` is the sole owner of entitlement + usage convergence. Every committed entitlement
  revision invokes the app-root usage convergence hook, and billing/quota UI callbacks await that
  one operation without issuing a second entitlement or usage request. The singleton
  `UsageService` invalidates its old summary and fetches fresh backend limits, including for deferred
  StoreKit updates that arrive with no billing sheet on screen. This summary refresh cannot clear or
  reinterpret an opaque pending-dispatch reservation or persisted ambiguous-reservation blockade.
- Deferred StoreKit transactions finish only after durable entitlement + usage convergence. A
  terminal cross-account ownership conflict or known deterministic IAP validation 4xx is also
  finished to avoid an unrecoverable redelivery loop. Transport, authentication, unknown 4xx, 5xx,
  and convergence failures remain unfinished for StoreKit or a later current-entitlement scan to
  recover. Direct purchases and deferred updates use the same finish policy; authority drift is a
  stale, unfinished disposition rather than permission to discard the transaction. The
  `SubscriptionStoreView` uses an app-owned subscribe control because its nonthrowing options
  provider cannot cancel a purchase; StoreKit is called only after exact authority validation and
  always receives the backend-issued `appAccountToken`. Deferred updates without a token cannot be
  claimed by the current account. Cached tokens are keyed to exact lifecycle/request authority, so
  A -> B and token rotation cannot reuse the prior account's StoreKit identity.
- Entitlement and usage operations capture immutable account ID, bearer token, backend URL, and a
  reset generation. Their 401 recovery may update or sign out only that still-current account;
  reset, sign-out, or account replacement retires suspended and queued work without recapturing a
  replacement account. Usage summary ordering remains independent from reservation authority.
...
- **`@Observable`**: Core state classes (`FocusSessionManager`, `IAPService`, `UsageService`) use `@Observable` instead of `@Published`.
