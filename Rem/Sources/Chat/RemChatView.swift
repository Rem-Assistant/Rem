...
/// is also rendered by `RemMac/Sources/UI/MacChatWindow.swift`. Both
...
struct RemChatView: View {
    ...
    /// Rem-runtime conversations remain usable without a personal gateway connection.
    ...
    var body: some View {
        SharedRemChatView(
            ...
            runtimeOwnsConversation: runtimeOwnsConversation,
    ...
    private func consumeQuotaSlot() async -> Bool {
        // Task continuations reserve their request atomically inside the Rem runtime.
        // Calling the legacy WebSocket `/usage/consume` preflight as well would charge
        // one visible message twice during the migration.
        if TaskChatSessionIdentity.taskId(from: viewModel.sessionKey) != nil {
            return true
        }
