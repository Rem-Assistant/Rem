...
struct OnboardingFlow: View {
                    ...
                    if previewShowsProviderButtons {
                        ...
                        VStack(spacing: DesignTokens.Spacing.md) {
                            SignInButton(
                                ...
                                action: {
                                    print("[Rem] Continue as returning user → confirmReturningUser()")
    ...
    private func continueAfterAuthentication(source: String) {
        print("[Rem] OnboardingFlow \(source) → recheckConfigured()")
        gateway.recheckConfigured()
        if gateway.isConfigured {
            print("[Rem] OnboardingFlow gateway.isConfigured=true → wakeAndConnectIfConfigured(), skip deploy")
    ...
    private func beginNewGatewaySetup() {
        if hasAcceptedAIDataSharing {
            print("[Rem] OnboardingFlow gateway not configured and consent already accepted → startDeploy()")
            step = .deploying
            startDeploy()
        } else {
            print("[Rem] OnboardingFlow gateway not configured → show AI data-sharing consent before deploy")
    ...
    private func beginDeployAfterConsent() {
        ...
        print("[Rem] OnboardingFlow consent accepted → startDeploy()")
        step = .deploying
        startDeploy()
    }
    ...
    private func finishPostSetupActivation() {
        ...
        print("[Rem] OnboardingFlow post-setup activation complete → release deploy gate")
    ...
    private func startDeploy() {
        print("[Rem] startDeploy() called → POST /api/v1/deploy")
        ...
        Task {
            do {
                ...
                print("[Rem] startDeploy() ok deployId=\(result.deployId) → polling status")
                try await pollStatus(deployId: result.deployId)
            } catch {
                print("[Rem] startDeploy() failed: \(error.localizedDescription)")
    ...
    private func pollStatus(deployId: String) async throws {
        ...
        while true {
            ...
            if status.phase != lastLoggedPhase {
                print("[Rem] deploy status phase=\(status.phase) message=\(status.message)")
                lastLoggedPhase = status.phase
            }

            switch status.phase {
            case "complete":
                print("[Rem] deploy complete → configuring gateway and waiting for connection")
                ...
                while Date().timeIntervalSince(connectionStart) < connectionTimeout {
                    ...
                    if gateway.connectionState.isConnected {
                        print("[Rem] gateway connected after deploy")
                        break
                    }
                }
                if !gateway.connectionState.isConnected {
                    print("[Rem] gateway connection timed out after deploy (will continue)")
                } else {
            ...
            case "failed":
                ...
                print("[Rem] deploy failed: \(status.message)")
                return
            default:
                continue
            }
