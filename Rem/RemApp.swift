...
struct RemApp: App {
    ...
    /// A validated `remclaw://connect` gateway-repoint request awaiting the user's explicit
    /// confirmation. Non-nil drives the "Connect to a new gateway?" alert; the repoint only reaches
    /// `gateway.configure(...)` on the user's own tap (#1342 PART 2).
    @State private var pendingGatewayConnect: GatewayConnectDeepLink.Request?
    ...
    private let isSuggestionsFixture: Bool = {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--rem-suggestions-fixture")
        #else
        false
        #endif
    }()
    /// Renders `RemConversationView` + `RemProposalCardView` (Rem's own conversation API surface)
    /// with canned task-update proposals in every lifecycle state — no gateway, no sign-in — so the
    /// proposal cards can be screenshotted headlessly. See `RemConversationProposalFixtureView`.
    private let isConversationProposalFixture: Bool = {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--rem-conversation-proposal-fixture")
        #else
        false
        #endif
    }()
    ...
    /// Renders a `[label](remclaw://connect?…)` link in real `AssistantMarkdownView` prose so the
    /// #1341 exploit hop — SwiftUI `Text` dispatching a custom-scheme link into the app — can be
    /// driven and proven closed by the `AssistantMarkdownLinkPolicy` allowlist (#1342 PART 1).
    private let isCustomSchemeLinkFixture: Bool = {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--rem-custom-scheme-link-fixture")
        #else
        false
        #endif
    }()
    ...
    init() {
        ...
        let iapService = IAPService(convergeUsage: { revision in
            usageService.invalidateSummary()
            do {
                try await usageService.fetchSummary()
                guard usageService.summary != nil else {
                    return .failed(
                        revision: revision,
                        message: "Rem couldn't refresh plan usage after the App Store change."
                    )
                }
                return .converged(revision: revision)
            } catch is CancellationError {
                return .stale(revision: revision)
            } catch {
                return .failed(revision: revision, message: error.localizedDescription)
            }
        })
        self._usageService = State(initialValue: usageService)
        self._iapService = State(initialValue: iapService)
        ...
        WindowGroup {
            rootContent
                ...
                .task {
                    ...
                    Task { await reconcileSubscriptionState() }
                ...
                .onChange(of: authService.isAuthenticated) { oldValue, newValue in
                    ...
                    if !oldValue && newValue {
                        ...
                        if authService.reconcileLocalDataOwnershipForSignedInUser() == .wipe {
                            ...
                            usageService.reset()
                            iapService.clearEntitlementState()
                        }
                ...
                // Explicit consent gate for a `remclaw://connect` gateway repoint. The deep-link
                // handler only ever stages a *validated* request here; the destructive
                // `gateway.configure(...)` runs solely on the user's "Connect" tap, and the host is
                // shown so they can see where their device is being pointed (#1342 PART 2).
                .alert(
                    "Connect to a new gateway?",
                    isPresented: Binding(
                        get: { pendingGatewayConnect != nil },
                        set: { if !$0 { pendingGatewayConnect = nil } }
                    ),
                    presenting: pendingGatewayConnect
                ) { request in
                    Button("Cancel", role: .cancel) { pendingGatewayConnect = nil }
                    Button("Connect") {
                        gateway.configure(
                            gatewayURL: request.gatewayURL,
                            gatewayToken: request.gatewayToken
                        )
                        pendingGatewayConnect = nil
                    }
                } message: { request in
                    Text("This will repoint Rem to \(request.gatewayHost) and connect using a supplied token. Only continue if you started this yourself from a trusted device.")
                }
        }
    ...
    private var rootContent: some View {
        ...
        } else if isCustomSchemeLinkFixture {
            NavigationStack {
                AssistantCustomSchemeLinkFixtureView()
            }
        ...
        } else if isSuggestionsFixture {
            NavigationStack {
                SuggestionSourceFixtureView()
                    .navigationTitle("Suggestions")
            }
        } else if isConversationProposalFixture {
            NavigationStack {
                RemConversationProposalFixtureView()
                    .navigationTitle("Task proposals")
            }
    ...
    private var isFixtureMode: Bool {
        isChatDiagnosticsFixture || isChatLifecycleFixture || isChatDayDividerFixture || isCustomSchemeLinkFixture || isSettingsFixture || isConnectorsFixture || isAutomationsFixture || isGatewayDetailFixture || isGatewayConnectionRecoveryFixture || isLaunchRecoveryCopyFixture || isRestoredSessionScrollFixture
            ...
            || isSuggestionsFixture
            || isConversationProposalFixture
    ...
    @MainActor
    private func reconcileSubscriptionState() async {
        ...
        await iapService.refreshEntitlementState()
    }
    ...
    private func handleDeepLink(_ url: URL) {
        ...
        // Gateway repoint (`remclaw://connect?url=…&token=…`). This hands a new backend the device's
        // node-command allowlist and chat transport, so it must NOT happen silently on a link the
        // user may not have knowingly followed (#1342 PART 2). Validate scheme/host fail-closed, then
        // stash the request for an EXPLICIT confirmation before `configure` runs. A malformed or
        // non-`https` payload is dropped here and never even prompts.
        guard let request = GatewayConnectDeepLink.request(from: url) else { return }
        pendingGatewayConnect = request
    }
