...
/// the session key and the live `RemChatViewModel` *instance* so the task
...
struct RemMainTabView: View {
    ...
    @State private var chatViewModel: RemChatViewModel?
    ...
    private var mainContent: some View {
        ...
        .onChange(of: gateway.mainSessionKey) { oldKey, newKey in
            ...
            if failedBriefRetryContext != currentExplicitBriefRetryContext() {
        ...
        VStack(spacing: 0) {
            ...
            NavigationStack(path: $navigationPath) {
                selectedTabContent
                    ...
                    .sheet(isPresented: Binding(
                        get: { usageService.quotaExceeded },
                        set: { if !$0 { usageService.dismissQuotaError() } }
                    )) {
                        QuotaExceededSheet()
                    }
                    .toolbar {
    ...
    private func setupChatViewModel(force: Bool = false) async -> Bool {
        ...
        let isRemRuntimeConversation = TaskChatSessionIdentity.taskId(from: key) != nil
        guard isRemRuntimeConversation || gateway.connectionState.isConnected else { return false }
        guard isRemRuntimeConversation || gateway.operatorReady else { return false }
        ...
        let transport = await gateway.makeChatTransport(
            ...
            taskContinuationProvider: { sessionKey, message, idempotencyKey in
                try await coordinator.continueConversation(
                    sessionKey: sessionKey,
                    message: message,
        ...
        return chatTransportSetupGate.commit(
            ...
            let viewModel = RemChatViewModel(
                sessionKey: key,
                transport: transport
            )
        ...
        Group {
            if let vm = agendaViewModel {
                AgendaView(viewModel: vm, onCreateTask: {
                   ...
                   canRetryBriefPlayback: talkMode.canRetryReadingAloud
                       && failedBriefRetryContext == currentExplicitBriefRetryContext(),
                   hasCompletedBriefPlayback: DailyBriefPlaybackReceipt.contains(
                    ...
                    let readState = DailyBriefReadControlState.resolve(
                        isReading: talkMode.isReadingAloud,
                        canRetry: talkMode.canRetryReadingAloud
                            && failedBriefRetryContext == currentExplicitBriefRetryContext()
                    )
                    switch readState {
                    case .stop:
        ...
        if let vm = chatViewModel {
            PersonalizedStarterChat(
                ...
                RemChatView(
                    ...
                    runtimeOwnsConversation: TaskChatSessionIdentity.taskId(from: sessionKey) != nil,
    ...
    private func openAndReadLatestBrief(
        ...
        let playbackTask = Task { @MainActor in
            ...
            if refreshedBrief.briefSessionKey == nil {
                // Gateway-free Rem-managed accounts still own the authenticated artifact. Read it
                // directly with the existing narration pipeline; no chat navigation/history is
                // implied, and the card's separate chat tap remains disabled.
                guard refreshedBrief.isAuthored,
                      dailyBriefPlaybackIdentity(for: agendaViewModel.brief) == refreshedPlaybackIdentity,
                      let retryContext = directBriefRetryContext(identity: refreshedPlaybackIdentity),
                      latestBriefPlayback.markBriefVoiceSessionStarted(for: requestID)
                else { return }
                failedBriefRetryContext = retryContext
                failedBriefRetryCompletion = {
                    ...
                    completedDailyBriefPlaybackReceipts = DailyBriefPlaybackReceipt.recording(
                        refreshedPlaybackIdentity,
                        in: completedDailyBriefPlaybackReceipts
                    )
                }
                let completed = await talkMode.readAloud(
                    refreshedAuthoredMarkdown,
                    retryContext: retryContext,
                    usesSystemVoiceOnly: true
                )
                if completed,
                   retryContext == failedBriefRetryContext,
                   retryContext == currentExplicitBriefRetryContext() {
                    failedBriefRetryCompletion?()
                    invalidateBriefRetryRecovery()
            ...
            let route = DailyOrchestratorChatRouting.conversationRoute(
                apiSessionKey: refreshedBrief.briefSessionKey
            )
            openDailyOrchestratorChat(
                route: route,
                briefMarkdown: nil,
    ...
    private func directBriefRetryContext(
        identity: DailyBriefPlaybackIdentity
    ) -> ExplicitSpeechRetryContext? {
        guard identity.accountID == authService.currentUser?.id,
              agendaViewModel?.brief?.isAuthored == true,
              agendaViewModel?.brief?.briefSessionKey == nil
        else { return nil }
        return ExplicitSpeechRetryContext(
            accountID: identity.accountID,
            gatewayID: "rem-runtime",
            sessionKey: "rem-runtime:\(identity.localDayKey)",
            localDayKey: identity.localDayKey,
            briefKey: identity.briefKey
        )
    }
    ...
    private func currentExplicitBriefRetryContext() -> ExplicitSpeechRetryContext? {
        guard let identity = dailyBriefPlaybackIdentity(for: agendaViewModel?.brief) else {
            return nil
        }
        if agendaViewModel?.brief?.briefSessionKey == nil {
            return directBriefRetryContext(identity: identity)
        }
        guard let sessionKey = gateway.mainSessionKey else { return nil }
    ...
    /// NavigationStack.
    ///
    /// Namespace caveat (root cause of the "empty linked chat" bug): for a
    /// cloud comment, `comment.sessionId` is a backend-generated run
    /// UUID (`randomUUID()` in `POST /tasks/:id/agent-run`, migration 022). That id
    /// is an execution identity — it is NOT a conversation key in the user's
    /// gateway, which is what legacy `chat.history` reads. The cloud run's
    ...
    ///
    /// Open the stable Rem-owned task conversation instead. Its persisted backend
    /// transcript is the continuation authority; gateway history is read-only
    /// compatibility data from turns created before this migration.
    ///
    ...
    private func openSession(for comment: TaskComment) {
        // Transitional autonomous sweeps still have a real, read-only OpenClaw recovery
        // transcript under `openclaw-sweep-task-<UUID>`. A tap on that exact Activity row
        // may inspect it; task composer/Continue actions still route to Rem's canonical chat.
        if comment.runtime == .gateway,
           let sessionKey = comment.sessionId, !sessionKey.isEmpty {
    ...
    /// Unified entry into a task's writable chat — used by the bottom composer and the
    /// "Continue in chat" CTA in the history view. New conversation turns always enter
    /// the task-scoped Rem transcript. A specific legacy gateway Activity row remains
    /// separately openable through `openSession(for:)` for recovery, but it must not
    /// select a different writable runtime for the task composer.
    ...
    /// Open (or reopen) a per-task Rem-owned continuation chat. The session key is derived from
    ...
    private func openTaskContinuationChat(taskId: String, latest: TaskComment?, draft: String = "") {
        // Continue on the canonical UUID session. The transport sends new turns to
        // Rem's backend and read-through merges the deterministic legacy `task-<12>`
        // gateway history. Do not choose identity
    ...
    private func stopBriefReadingAndContinue() {
        invalidateBriefRetryRecovery()
        let hasConversation = agendaViewModel?.brief?.briefSessionKey != nil
        talkMode.stopReadingAloud(continueListening: hasConversation)
        if !hasConversation {
            latestBriefPlayback.endVoiceSession()
        }
    }
