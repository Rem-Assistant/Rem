...
| `MacRemConversationApiService.swift` | macOS mirror of the iOS `RemConversationApiService`. REST client for Rem's own conversation API (`/api/v1/conversations/*`) — create/list/get, `sendChat`, and proposal approve/dismiss — routed through `MacAuthenticatedHttpClient` (JWT auth, base URL, 401 refresh). Conforms to the Shared `RemConversationApiProviding` so the shared `RemConversationViewModel`/`RemConversationView` bind to it. Injectable `MacRemConversationHttpClient` seam makes it unit-testable with a mock transport; maps non-2xx to `RemConversationApiError.requestFailed`, preserving structured `reason`/`failure_code`. |
