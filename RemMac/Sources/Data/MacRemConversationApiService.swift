...
//
// macOS mirror of the iOS `RemConversationApiService`
// (`RemClaw/Sources/Services/RemConversationApiService.swift`). Same request shaping, decoding, and
// error mapping; the only difference is the transport — every call routes through
// `MacAuthenticatedHttpClient` (`RemClawMac/Sources/Gateway/MacAuthenticatedHttpClient.swift`) so
// this service inherits the Mac client's JWT auth header, base-URL resolution, and silent 401
// refresh, exactly as the Mac REST clients `MacTaskStore` / `MacOrchestratorSuggestionStore` do.
//
// The iOS seam protocol (`RemConversationHttpClient`) is defined in the iOS-only target file, so it
// is not visible to the Mac module; this file declares the equivalent Mac seam
// (`MacRemConversationHttpClient`) around the static entry point so the service is unit-testable
// with a mock transport (no auth, no network) — mirroring the iOS test seam.

/// The single HTTP operation `MacRemConversationApiService` needs. Production wraps
/// `MacAuthenticatedHttpClient.request`; tests inject a mock.
@MainActor
protocol MacRemConversationHttpClient {
    func send(path: String, method: String, body: Data?) async throws -> (Data, HTTPURLResponse)
}

/// Production transport: authenticated request via the shared Mac client, so this service inherits
/// its auth header, base-URL resolution, and silent 401 refresh.
@MainActor
struct MacAuthenticatedConversationHttpClient: MacRemConversationHttpClient {
    func send(path: String, method: String, body: Data?) async throws -> (Data, HTTPURLResponse) {
        try await MacAuthenticatedHttpClient.request(path: path, method: method, body: body)
    }
}
...
/// mutations go through the JWT-authenticated shared Mac HTTP client. macOS mirror of the iOS
/// `RemConversationApiService`; both conform to the Shared `RemConversationApiProviding` so the
/// shared `RemConversationViewModel` / `RemConversationView` bind to either platform's concrete client.
@MainActor
final class MacRemConversationApiService: RemConversationApiProviding {
    private let httpClient: MacRemConversationHttpClient
    private let decoder = JSONDecoder()
    private let encoder = JSONEncoder()

    private static let basePath = "/api/v1/conversations"

    init(http: MacRemConversationHttpClient) {
        self.httpClient = http
    }

    /// Production initializer: routes through the authenticated shared Mac client. Kept separate from
    /// the injectable initializer because a `@MainActor` default-argument expression evaluates in a
    /// nonisolated context and cannot call `MacAuthenticatedConversationHttpClient()`.
    convenience init() {
        self.init(http: MacAuthenticatedConversationHttpClient())
    }
