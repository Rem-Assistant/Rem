...
///
/// Uses the same GatewayNodeSession from RemKit for both roles.
actor MacGatewayClient {
    ...
    ///   - isBootstrap: when `true`, `token` is routed to RemKit's
    ...
    /// Forgets the device-auth pairing tokens stored on disk by RemKit,
