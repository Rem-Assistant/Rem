...
final class LocalGatewayManager {
    ...
    /// (in-app Settings, `LocalGatewaySetupView`, `RemMacApp`,
    ...
    ///
    /// Also strips the legacy `"ai"` top-level key that earlier Rem builds
    ...
    func installCLI() async {
        status = .installingCLI("Installing gateway CLI...")
    ...
    func installLaunchAgent(port: Int = defaultPort) async throws {
        guard let path = cliPath else {
            throw NSError(domain: "LocalGateway", code: -1,
                          userInfo: [NSLocalizedDescriptionKey: "Gateway CLI not installed"])
        }
    ...
    /// and RemKit's `GatewayChannel` exchanges it for a persistent
    ...
    func start(port: Int = defaultPort, token _: String? = nil) async {
        guard let cliPath else {
            status = .failed("Gateway CLI not installed")
            return
        }
        ...
        let logsDir = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Logs/Rem")
