...
nonisolated private final class MacTransportState: @unchecked Sendable {
    ...
    private var _finalMessage: RemKit.AnyCodable?
    ...
    var finalMessage: RemKit.AnyCodable? {
    ...
    func consumeFinalState(for sessionKey: String) -> (finalMessage: RemKit.AnyCodable, userText: String?)? {
        lock.withLock {
...
private final class MacChatMemoryDiagnostics: @unchecked Sendable {
    ...
    nonisolated func recordHistory(payload: RemChatHistoryPayload, rawBytes: Int) {
    ...
    nonisolated func recordYield(_ result: AsyncStream<RemChatTransportEvent>.Continuation.YieldResult, kind: String) {
        lock.withLock {
            let didDrop: Bool
            switch result {
            case .enqueued:
...
/// Implements RemChatTransport using the operator session for chat RPCs
/// and the event stream, at parity with the iOS IOSGatewayChatTransport.
struct MacChatTransport: RemChatTransport, Sendable {
    ...
    /// stable history session id expected by `RemChatViewModel`.
    ...
    static func routedChatEvent(
        ...
        guard let rewrittenData = try? JSONSerialization.data(withJSONObject: dict),
              let rewritten = try? JSONDecoder().decode(RemChatEventPayload.self, from: rewrittenData)
        else { return payload }
        return rewritten
    }
    ...
    static func decodeModelChoices(from data: Data) throws -> [RemChatModelChoice] {
    ...
    private static func modelChoices(from result: ModelsListPayload) -> [RemChatModelChoice] {
    ...
    func requestHistory(sessionKey: String) async throws -> RemChatHistoryPayload {
        ...
        let json = String(data: data, encoding: .utf8)
        let rawPayload: RemChatHistoryPayload
        do {
            let res = try await gateway.request(method: "chat.history", paramsJSON: json, timeoutSeconds: 15)
            rawPayload = try JSONDecoder().decode(RemChatHistoryPayload.self, from: res)
            #if DEBUG
    ...
    func sendMessage(
        ...
        do {
            ...
            let acknowledgement = try await MacQuotaGatewayDispatch.run(
                ...
                decodeRunID: { data in
                    try JSONDecoder().decode(RemChatSendResponse.self, from: data).runId
                },
                ...
                for await evt in stream {
                    ...
                    case "agent":
                        ...
                        guard let decodedPayload = try? GatewayPayloadDecoding.decode(
                            payload, as: RemAgentEventPayload.self) else { break }
    ...
    private func yieldEvent(
        _ event: RemChatTransportEvent,
        kind: String,
        continuation: AsyncStream<RemChatTransportEvent>.Continuation
    ) {
    ...
    private func cleanHistoryMessages(
        _ payload: RemChatHistoryPayload
        ...
        do {
            ...
            let patchedData = try JSONSerialization.data(withJSONObject: dict)
            return try JSONDecoder().decode(RemChatHistoryPayload.self, from: patchedData)
        } catch {
            return nil
        }
    }
    ...
    private func patchHistoryIfStale(
        payload: RemChatHistoryPayload,
        cachedMessage: RemKit.AnyCodable,
