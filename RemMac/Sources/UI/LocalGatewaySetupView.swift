...
struct LocalGatewaySetupView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 8) {
                ...
                Text("Run a local gateway on this Mac -- no cloud needed.")
                    .font(.callout)
    ...
    private var installStep: some View {
        VStack(spacing: 16) {
            stepHeader(number: 1, title: "Install gateway CLI")

            Text("The gateway CLI is required to run a local gateway.")
                .font(.callout)
                .foregroundStyle(.secondary)
