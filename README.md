...
> **Running without API keys or an account:** a clean clone **builds** the iOS,
> Android, and macOS apps and the backend with no secrets. You cannot sign in or
> use the live assistant without a Rem account (see [CONTRIBUTING.md](CONTRIBUTING.md)
> — "what you can and cannot run today"); iterate on iOS UI with the `--rem-*-fixture` debug launch
